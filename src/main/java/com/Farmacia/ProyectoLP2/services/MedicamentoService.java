...
public class MedicamentoService {
	...
	public List<Medicamento> search(MedicamentoFilter filter) {
		return medicRepository.findAllWithFilters(filter.getIdCategoria(), filter.getIdProveedor(),filter.getPreescripcion());
	}
	...
	public ResultadoResponse create(Medicamento m) {
		try {
			...
			medicRepository.createMedicine(m.getImagenBytes(), m.getNombre(), m.getPrecio(), m.getStockActual(),
					m.getFechaVencimiento(), m.getDescripcion(), m.getProveedor().getIdProveedor(),
					m.getPreescripcion(), m.getCategoria().getIdCategoria());
	...
	public ResultadoResponse update(Medicamento m) {
		try {
			...
			if (m.getImagen() != null && !m.getImagen().isEmpty()) {
				nuevaImagenBytes = m.getImagen().getBytes();
			} else {
				Medicamento original = medicRepository.findById(m.getIdMedicamento()).orElse(null);
				if (original != null) {
					nuevaImagenBytes = original.getImagenBytes();
				}
			}

			m.setImagenBytes(nuevaImagenBytes);

			medicRepository.updateMedicine(m.getIdMedicamento(), m.getImagenBytes(), m.getNombre(), m.getPrecio(),
					m.getStockActual(), m.getFechaVencimiento(), m.getDescripcion(), m.getProveedor().getIdProveedor(),
					m.getPreescripcion(), m.getCategoria().getIdCategoria());

			return new ResultadoResponse(true, "Medicamento actualizado correctamente");
